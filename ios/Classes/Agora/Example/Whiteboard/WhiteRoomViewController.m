...
{
    UIBarButtonItem *item2 = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Sgare", nil) style:UIBarButtonItemStylePlain target:self action:@selector(shareRoom:)];
...
- (void)joinNewRoom
{
    self.title = NSLocalizedString(@"Creating Room...", nil);
...
- (void)joinExistRoom
{
    ...
    [WhiteUtils getRoomTokenWithUuid:self.roomUuid completionHandler:^(NSString * _Nullable roomToken, NSError * _Nullable error) {
        if (roomToken) {
            ...
            UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"获取 RoomToken 失败", nil) message:[NSString stringWithFormat:@"error message:%@", [error description]] preferredStyle:UIAlertControllerStyleAlert];
...
{
    self.title = NSLocalizedString(@"Connecting to the room", nil);
...
{
    self.title = NSLocalizedString(@"Whiteboard Bar", nil);
    self.roomToken = roomToken;
    self.room = room;
...
- (void)defaultActionAfterJoinRoomError:(NSError *)error
{
    ...
    UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Join Room Failed", nil) message:[NSString stringWithFormat:@"error message:%@", [error localizedDescription]] preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *action = [UIAlertAction actionWithTitle:NSLocalizedString(@"确定", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        [self.navigationController popViewControllerAnimated:YES];
    }];
    [alertVC addAction:action];
